            ...
            <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-4 rounded-2xl bg-dark-canvas/70 px-6 py-6 ring-1 ring-dark-border">
              {modelProviders.map((p) => (
                <div key={p.name} className="flex items-center">
                  {p.name === 'OpenAI' ? (
                    // Inline currentColor OpenAI mark: inherits the ambient
                    // (theme-aware) text colour, so it reads light on the dark
                    // trust-layer row AND dark on any light surface. The old
                    // /icons/openai.svg was baked white and vanished on light
                    // backgrounds; inlining also drops the network fetch.
                    <svg
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      role="img"
